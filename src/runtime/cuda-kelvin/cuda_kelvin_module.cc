...
class CUDAKelvinModuleNode : public runtime::ModuleNode {
 public:
  explicit CUDAKelvinModuleNode(std::string data, std::string fmt,
  ...
  std::string GetSource(const std::string& format) final {
    ...
    if (cuda_source_.length() != 0) {
      ...
      if (fmt_ == "ptx_kelvin") return data_;
      return "";
    }
  }
...
class CUDAKelvinWrappedFunc {
 public:
  // initialize the CUDA function.
  void Init(CUDAKelvinModuleNode* m, ObjectPtr<Object> sptr, const std::string& func_name,
...
class CUDAKelvinPrepGlobalBarrier {
 public:
  CUDAKelvinPrepGlobalBarrier(CUDAKelvinModuleNode* m, ObjectPtr<Object> sptr) : m_(m), sptr_(sptr) {
...
PackedFunc CUDAKelvinModuleNode::GetFunction(const std::string& name,
  ...
  if (name == symbol::tvm_prepare_global_barrier) {
    return PackedFunc(CUDAKelvinPrepGlobalBarrier(this, sptr_to_self));
  }
...
Module CUDAKelvinModuleCreate(std::string data, std::string fmt,
                              std::unordered_map<std::string, FunctionInfo> fmap,
                              std::string cuda_source) {
  auto n = make_object<CUDAKelvinModuleNode>(data, fmt, fmap, cuda_source);
  return Module(n);
}

// Load module from module.
Module CUDAKelvinModuleLoadFile(const std::string& file_name, const std::string& format) {
  std::string data;
  std::unordered_map<std::string, FunctionInfo> fmap;
  std::string fmt = GetFileFormat(file_name, format);
  std::string meta_file = GetMetaFilePath(file_name);
  ...
  return CUDAKelvinModuleCreate(data, fmt, fmap, std::string());
}

Module CUDAKelvinModuleLoadBinary(void* strm) {
...
TVM_REGISTER_GLOBAL("runtime.module.loadfile_ptx_kelvin").set_body_typed(CUDAKelvinModuleLoadFile);

TVM_REGISTER_GLOBAL("runtime.module.loadbinary_cuda_kelvin").set_body_typed(CUDAKelvinModuleLoadBinary);
}  // namespace runtime
}  // namespace tvm
